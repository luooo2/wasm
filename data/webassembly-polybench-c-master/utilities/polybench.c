...
double rtclock()
{
    ...
    /*
     * gettimeofday() is not available in WASI libc (and may even be
     * undeclared under the wasi-sdk sysroot). Use clock_gettime()
     * when targeting WASI.
     */
#if defined(__wasi__)
    struct timespec ts;
    int stat = clock_gettime(CLOCK_MONOTONIC, &ts);
    if (stat != 0)
      printf ("Error return from clock_gettime: %d", stat);
