...
For a single program we:
  1. compute the canonical reference output from the native binary;
  2. benchmark the native binary (for the wasm/native ratio);
  3. for each strategy (raw / O2 / O3 / random / llm_static / llm_static_perf):
       a. build the candidate wasm (wasm-opt);
       b. wasm-validate it;
       c. run it on wasmer and check output == reference (differential test);
       d. benchmark median internal runtime over N repeats.
  4. decide accept/reject per the PoC rule:
       accept iff validate ok AND output matches AND
       median runtime improves by >= MIN_SPEEDUP vs the chosen reference.

Returns a list of per-strategy result dicts. Used by s07_run_all.
"""
...
def evaluate_program(
    ...
    # baseline (raw) wasm runtime is the per-program reference for "speedup".
    ...
    for strat in strategies:
        ...
        # differential correctness vs native reference
    ...
    # second pass: fill speedup_vs_O3 and acceptance now that raw & O3 are known
    o3_ms = next(
    ...
    for r in rows:
        ...
        # acceptance: correct + validated + >=3% faster than raw baseline
