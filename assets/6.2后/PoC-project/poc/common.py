#!/usr/bin/env python3
"""Shared helpers for the LLM-guided wasm-opt pass-selection PoC.

Centralises:
  - path resolution (repo root, llvm_direct build dir, project dirs);
  - the wasmer / native run + internal-timer (TIME_NS) measurement harness;
  - wasm-opt / wasm-validate wrappers;
  - output normalisation for differential correctness testing;
  - small statistics utilities.

The benchmark methodology mirrors the existing wasmer migration scripts:
  native : run the ELF binary directly;
  wasm   : `wasmer compile --cranelift x.wasm -o x.wasmu` once, then
           `wasmer run x.wasmu` repeatedly (AOT, so the measured region is
           steady-state execution rather than JIT warm-up).

Each benchmark prints `TIME_NS:<ns>` for the timed compute region; we use that
internal timer as the primary signal and the external wall-clock as a sanity
check.
"""
...
# Paths
# --------------------------------------------------------------------------- #
PROJECT_ROOT = Path(__file__).resolve().parent.parent          # .../PoC-project
REPO_ROOT = PROJECT_ROOT.parents[2]                            # .../wasm
BUILD_DIR = REPO_ROOT / "data" / "build" / "llvm_direct"
...
for _d in (WORK_DIR, PROMPT_DIR, LLM_RESP_DIR, CAND_DIR, RESULTS_DIR, FIG_DIR):
...
# wasmer prints a few non-deterministic warnings to stderr; ignore for diffing.
_NOISE_PREFIXES = (
    "warning:",
    "warning :",
    "Compiling",
